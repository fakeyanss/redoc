...
export const SidebarCollapseButton = () => {
  return (
    <SidebarCollapseContainer className="sidebar-callapse" onClick={collapse}>
      <SidebarCollapseSvg />
...
export const SidebarExpandButton = () => {
  return (
    <SidebarExpandContainer className="sidebar-expand" onClick={expand}>
      <SidebarExpandSvg />
...
const SidebarCollapseContainer = styled.div`
  ${function ({ theme }) {
    ...
    rightPanelWidth = `${theme.rightPanel.width}`;
    return `
      margin-left: calc(${theme.sidebar.width} - 12px);
      top: ${theme.sidebar.collapseBtn.top};
      z-index: 10;
      width: 24px;
      height: 24px;
      background: #fff;
...
const SidebarCollapseSvg = ({ className = '' }) => (
  <svg
    className={className}
    style={{
      textAlign: 'center',
...
const SidebarExpandSvg = ({ className = '' }) => (
  <svg
    className={className}
    style={{
      textAlign: 'center',
...
const collapse = () => {
  const sidebarExpand = document.getElementsByClassName('sidebar-expand')[0] as HTMLElement;
  const sidebarCollapase = document.getElementsByClassName('sidebar-callapse')[0] as HTMLElement;
...
function findFirstVisibleElement(container: Element): Element | null {
  ...
  // Handling Cases:
  // The current location hash corresponds to a DOM content that has almost entirely been scrolled above the viewport,
  // except for the operation's ending delimiter line at the bottom.
